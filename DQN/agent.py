...
def DeepQNetwork(lr, num_actions, input_dims, fc1):
    ...
    q_net.add(Conv2D(filters=64, kernel_size=4, strides=2, activation='relu', data_format='channels_last'))
    q_net.add(Conv2D(filters=64, kernel_size=3, strides=1, activation='relu', data_format='channels_last'))
    q_net.add(Flatten())
    ...
    def policy(self, observation):
        if np.random.random() < self.epsilon:
            ...
            if type(observation) == tuple:
                state = np.array([observation[0]], copy=False, dtype=np.float32)
            else:
    ...
    def train_model(self, env, num_episodes, graph):
        ...
        for i in range(num_episodes):
            ...
            while not done:
                ...
                new_state, reward, done, sample_bool, _ = env.step(action)
                steps += 1
                score += reward
            ...
            print("Episode {0}/{1}, Score: {2} ({3}), AVG Score: {4}, Steps: {5}".format(i, num_episodes, score,
                                                                                         ...
                                                                                         steps))
            with open('all_episodes.txt', 'a') as ae:
                ae.write("Episode {0}/{1}, Score: {2} ({3}), AVG Score: {4}, Steps: {5}\n".format(i, num_episodes, score,
                                                                                         self.epsilon, avg_score,
                                                                                         steps))
            if score == 21:
                self.q_net.save(("saved_networks1/dqn_model{0}".format(f)))
                self.q_net.save_weights(("saved_networks1/dqn_model{0}/net_weights{0}.h5".format(f)))
